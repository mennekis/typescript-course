...
function exercise41() {
   ...
   function printAllUserPropTypes() {
      ...
      keys.forEach((key) => {
         const propertyType = typeof user[key];
...
function exercise42() {
   ...
   type TCoordinates =
      (typeof products)[number]["warehouse"]["address"]["coordinates"];
   ...
   function getProductWarehousePhoneNumber(
      product: TProduct
   ): (typeof product)["warehouse"]["phoneNumbers"][number] {
...
function exercise46() {
   ...
   type FirstParameter<T> = T extends (...args: infer Args) => any
      ? Args[0]
      : never;

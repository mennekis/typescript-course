function lesson8() {
  ...
  function lookupTypes() {
    ...
    type TCity = TUser["address"]["city"]; // string
    ...
    type keys = "name" | "age";
    type PX = Person[keys]; // string | number
    //   let x = "name"
  ...
  function keyofTypeOperator() {
    ...
    function getPropertyV4(obj: TPersonV1, key: keyof TPersonV1) {
      console.log("getPropertyV2", obj[key]);
    ...
    // const parsonAddress2 = getPropertyV6(person7, 'adres'); // error for typo
  ...
  function conditionalTypes() {
    ...
    type TIsNumber2 = IsNumber<0>; // number
    type TIsNumber3 = IsNumber<typeof x>; // number
  ...
  function coditionalTypesWithUnionsAndNever() {
    ...
    type TNotNullable = NotNullable<TNullableUser>; // TUser
    ...
    let confirmMessage: number | null | undefined;
    // fetch().then()
    let confirmMessage2: NotNullable<typeof confirmMessage> = 123;

    // if utility generic conditional type returns never for some type in the union,
    // it is removed from the union
    type colors = "red" | "green" | "blue" | "yellow";

    type TExcludeRed<T> = T extends "red" ? never : T;
    type colorsWithoutRed = TExcludeRed<colors>; // 'green' | 'blue' | 'yellow'
  ...
  function inferKeywordAndReturnTypeUtility() {
    ...
    type TTypeOfArray5 = TGetTypeOfArray<typeof userList>; // TUser
    ...
    type X = Parameters<typeof X>; // [string, number]
    const x: X = ["abc", 123];
  }
